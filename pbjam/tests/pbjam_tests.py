...
class case():
    ...
    def load_prior(self,):
        ...
        if self.ID == 'silly':
            self.st.prior = type('kde', (object,), {})()
            self.st.prior.samples = np.ones((2,10))
            self.st.prior.prior_data = np.ones((2,10))

            data = np.array(self.pars['asypars']).repeat(11).reshape((10,-1))
            self.st.prior.kde = sm.nonparametric.KDEMultivariate(data=data, var_type='cccccccccc', bw='scott')
